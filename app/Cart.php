...
class Cart {
    ...
    public function add($item, $id) {
        $storeditem = ['qty' => 0, 'price' => $item->price,'unit_price' => $item->price, 'item' => $item];
        if ($this->items) {
    ...
    public function updateCartItem($id, $index, $item_type) {
        //echo $id."-".$index."-".$item_type; die();
        $storeditem = [];
        if ($this->items) {
            if($item_type == 1)
            {
    ...
    public function deductCartItem($id, $index, $item_type) {
        //echo $id."-".$index."-".$item_type; die();
        $storeditem = [];
        if ($this->items) {
            if($item_type == 1)
