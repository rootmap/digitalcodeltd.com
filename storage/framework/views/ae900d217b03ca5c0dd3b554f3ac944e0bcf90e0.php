            ...
            $('.addtocartim').click(function(){
                ...
                $.each($('.imj'+tagID),function(k,r){
                    ...
                    if(getOptVal!=0)
                    {
                        ...
                        var optPrice=$('select[name='+optnName+'] option:selected').attr('data-price');
                        getOptPrice+=(optPrice-0);
                        flavour_quantity+=1-0;
                    }
                });

                var total_select_opt=0;
                $.each($('.imj'+tagID),function(k,r){
                    total_select_opt+=1-0;
                });
                console.log('Total Select Option = ',total_select_opt);
                console.log('Product desktop Name Param',getOptNameParam);

                if(flavour_quantity!=total_select_opt){
                    Swal.fire({
                        icon: 'error',
                 ...
                 $.each($(".mini-cart-list").children('li'),function(k,r){
                    ...
                    if($(r).attr('data-id'))
                    {
                        if($(r).attr('data-id')==prodRow)
                        {
                            ...
                            $(r).children('input').val(newQuantity);
    ...
    <script type="text/javascript">
        ...
        function increasendecrease(item_id, item_index, inclusive, addtoCartURL)
        {
            $.ajax({
                ...
                'dataType': 'json',
                'url': addtoCartURL,
                'data': {'item_id':item_id,'item_index':item_index,'inclusive':inclusive,'_token':"<?php echo e(csrf_token()); ?>"},
                'success': function (data) {
                    loadCart(data);
                }
            });
        ...
        $('body').on('click','.increase_cart_item',function(){
            console.log($(this).parent().html());
            var data_inclusive=$(this).parent().attr('data-inclusive');
            var inclusive=0;
            if(data_inclusive==1)
            {
                inclusive=1;
            }
            var item_id=$(this).parent().attr('item-id');
            var item_index=$(this).parent().attr('data-index');
            console.log(item_index, item_id, inclusive);
            var addtoCartURL="<?php echo e(url('order-item/increase-to-cart')); ?>";
                //------------------------Ajax POS Start-------------------------//
            increasendecrease(item_id, item_index, inclusive, addtoCartURL);
            //data-inclusive
        });

        $('body').on('click','.decrease_cart_item',function(){
            console.log($(this).parent().html());
            var data_inclusive=$(this).parent().attr('data-inclusive');
            var inclusive=0;
            if(data_inclusive==1)
            ...
            var item_index=$(this).parent().attr('data-index');
            console.log(item_index, item_id, inclusive);
            var addtoCartURL="<?php echo e(url('order-item/deduct-to-cart')); ?>";
                //------------------------Ajax POS Start-------------------------//
            increasendecrease(item_id, item_index, inclusive, addtoCartURL);
            //data-inclusive
        });
